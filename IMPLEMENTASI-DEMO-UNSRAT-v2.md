...
**Catatan optimisasi opsional (bukan wajib untuk scope pekerjaan ini)**: pola `path.read_text()` yang dipakai `root()` saat ini bersifat *synchronous blocking* di dalam `async def`. Untuk demo lokal single-user ini dampaknya praktis tidak terasa (bukan risiko kritis untuk konteks sidang), tapi kalau ingin dibenahi sebagai best practice FastAPI, gunakan `FileResponse` dari `fastapi.responses` yang menangani pembacaan file secara async beserta MIME type otomatis. **Jika dilakukan, terapkan konsisten ke SEMUA route serving HTML** (`/`, `/testing`, `/evaluation`, `/dev`) sekaligus — bukan hanya route baru — supaya tidak ada inkonsistensi pola antara kode lama dan baru. Ini di luar scope wajib dokumen ini; lakukan hanya jika ada waktu luang setelah semua fase inti selesai dan lulus testing.
...
3. **Semua path asset baru (JS/CSS) wajib absolut, mengikuti mount point `/static`**: `/static/demo/js/chat-widget.js`, `/static/demo/css/demo-modal.css`, dst. **Jangan pakai path relatif** (`js/chat-widget.js`) — karena halaman ini disajikan dari route `/`, path relatif akan di-resolve browser ke root domain (`/js/chat-widget.js`), bukan ke lokasi file sebenarnya, dan menghasilkan 404.
4. Sisipkan tombol trigger baru (`#rag-trigger-btn`): posisi `fixed`, `bottom-right`, ditumpuk di atas elemen `.smooth-back-to-top-button` yang sudah ada (beri jarak/margin agar tidak tumpang tindih — cek posisi persis elemen back-to-top itu di CSS asli sebelum menentukan offset), dengan ikon berbeda (misal ikon `message-square` dari lucide) agar semantiknya jelas berbeda dari back-to-top maupun AddToAny.
5. **Z-index eksplisit tinggi wajib** untuk `#rag-chatbot-widget`, `#rag-trigger-btn`, dan `#rag-modal` (misal `z-[999999]` via Tailwind arbitrary value), bukan kelas standar seperti `z-50`. Tema WordPress kompleks ini (RevSlider, sticky navbar, AddToAny) lazim memakai z-index tinggi juga — tanpa nilai eksplisit besar, modal/tombol berisiko tertutup elemen lain.
6. Bangun markup modal (`#rag-modal`) mengikuti **gaya visual `tab-chat` di `index.html`** (palet maroon `#7B2D2D`/`#963E3E`, bubble style, welcome message dengan quick-question buttons, custom scrollbar) — **tanpa sidebar**. Karena tidak ada sidebar:
   - Header modal dibuat **bersih**: hanya judul ("Asisten Akademik UNSRAT"), status badge kecil, dan tombol close (X).
   - `config-select`/`model-select` **tidak** ditaruh langsung di header (berisiko padat/bertumpuk di layar kecil). Sediakan ikon pengaturan (gear icon) kecil di header yang membuka panel dropdown/accordion ringan berisi kedua select tersebut saat diklik.
   - Referensi elemen persis yang perlu direplikasi gayanya: baris 148–203 `index.html` (area `tab-chat`) untuk body modal, dan baris 93–125 `index.html` (bagian config/model dropdown di sidebar) untuk dipindah ke panel pengaturan modal.
7. Modal default `hidden` (via Tailwind `class="hidden"` atau setara), baru tampil saat tombol trigger diklik (toggle class, bukan reload halaman, bukan iframe).
8. **Scroll bleeding**: saat modal dibuka, tambahkan class `overflow-hidden` ke `document.body` (dan hapus saat modal ditutup), supaya scroll di halaman WordPress di belakang modal tidak ikut bergerak. Tambahkan juga `overscroll-behavior: contain` pada `#rag-chat-messages` supaya scroll riwayat chat yang mencapai batas atas/bawah tidak "bocor" ke scroll body.
9. Ini adalah **desain tahap awal** — struktur/style boleh diiterasi lebih lanjut setelah semua fungsi terbukti bekerja. Prioritas Fase 2 adalah kelengkapan fungsi, bukan kesempurnaan visual.
...
- **Ikon (lucide)**: `safeCreateIcons()` di kode asli dipanggil per-event diskrit (saat bubble baru dibuat, citation di-render, atau abort) — **bukan** per-token streaming, jadi tidak ada masalah performa signifikan dari pemanggilan berulang. Namun karena halaman host (`unsrat-ac-id.html`) sangat besar (1600+ baris, banyak ikon SVG bawaan tema), idealnya scan Lucide dibatasi hanya ke dalam `#rag-chatbot-widget`, bukan seluruh dokumen. **Verifikasi dulu ke dokumentasi resmi lucide (via context7 atau sumber resmi lain, lihat Bagian 9) apakah build UMD yang dipakai (`unpkg.com/lucide@latest`) mendukung opsi scoping semacam `root`** sebelum menulis kode yang mengasumsikan API tersebut ada — jangan menebak nama parameter API. Kalau ternyata tidak didukung di versi yang dipakai, cukup panggil `lucide.createIcons()` global seperti pola existing; ini bukan blocker kritis, hanya optimisasi opsional.
- **Penanganan saat modal ditutup ketika stream masih berjalan**: default yang dipakai adalah **abort stream** (`abortController.abort()`) saat `closeModal()` dipanggil di tengah streaming — paling sederhana dan konsisten dengan prinsip minim kompleksitas untuk tahap awal. Menyimpan histori chat ke `sessionStorage` agar tetap ada saat modal dibuka ulang/halaman di-refresh adalah **future improvement**, bukan bagian scope Fase 3 — jangan diimplementasikan kecuali diminta eksplisit, supaya scope tetap terkendali.
...
**Reset minimal yang tetap diperlukan di dalam widget**: markup modal (mengikuti gaya `tab-chat`, lihat Fase 2) sudah didesain dengan styling eksplisit lengkap (`appearance-none`, `focus:outline-none`, dst pada input/select), jadi risiko elemen "kelihatan aneh" akibat Preflight mati sudah banyak termitigasi oleh desainnya sendiri. Yang tetap perlu ditambahkan secara eksplisit hanyalah `box-sizing: border-box`, karena tanpa Preflight browser default ke `content-box`, sementara seluruh sistem width/height/padding Tailwind mengasumsikan `border-box` — mismatch ini bisa membuat ukuran elemen sedikit meleset. Tambahkan di `demo-modal.css`:
```css
#rag-chatbot-widget, #rag-chatbot-widget *, #rag-chatbot-widget *::before, #rag-chatbot-widget *::after {
  box-sizing: border-box;
}
```
**Jangan** menambahkan reset CSS yang lebih luas dari ini (margin, border-width, dsb) di tahap awal — markup modal sudah eksplisit di-styling per elemen mengikuti pola `tab-chat`, jadi reset tambahan berisiko menimbulkan efek samping baru yang justru harus di-debug ulang. Kalau nanti saat testing visual (Fase 6) ditemukan elemen spesifik yang masih terpengaruh gaya bawaan browser/tema, tambahkan reset **per-elemen yang terbukti bermasalah**, bukan reset generik di muka.
