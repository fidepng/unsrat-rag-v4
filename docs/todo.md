...
*   **Current Research Stage:** 🧪 Data Expansion & Official Evaluation Prep (Post-Task 11)
...
*   [x] **Task 1: Project Foundation & Environment**
    *   Conda environment, dependencies ([environment.yml](file:///D:/Kuliah/Skripsi%20Repository/unsrat-rag-v4-28.05.2026/environment.yml)), and environment variables (`.env`) established.
*   [x] **Task 2: src/config.py — Pusat Konfigurasi**
    *   Created [config.py](file:///D:/Kuliah/Skripsi%20Repository/unsrat-rag-v4-28.05.2026/src/config.py) as the single source of truth for all parameters.
...
*   [x] **Task 9: static/ — SPA Frontend**
    *   Built Vanilla HTML/CSS/JS frontend with dual-tabs, thinking indicators, and comparison charts.
*   [x] **Task 10: evaluation.py — Pipeline Evaluasi Ragas**
    *   Implemented Ragas metrics evaluation, Wilcoxon signed-rank test calculation, and Chart.js integration.
...
*   [x] **Task 11: Kalibrasi SIMILARITY_THRESHOLD (D-B7)**
    *   [x] **Step 11.1:** Write empirical calibration script ([calibrate_threshold.py](file:///D:/Kuliah/Skripsi%20Repository/unsrat-rag-v4-28.05.2026/scripts/calibrate_threshold.py)) to evaluate retrieval accuracy across multiple thresholds (e.g., 0.55, 0.60, 0.65, 0.70).
    *   [x] **Step 11.2:** Run calibration using the current ground truth.
    *   [x] **Step 11.3:** Analyze recall/precision and verify if `SIMILARITY_THRESHOLD` in [config.py](file:///D:/Kuliah/Skripsi%20Repository/unsrat-rag-v4-28.05.2026/src/config.py) needs adjustment.
        *   *Result:* Empirically verified a perfect decision gap of **0.117** with a decision boundary midpoint at **0.291**. The current config value of `0.3` is highly optimal (0.009 deviation) and is officially validated.
    *   [x] **Step 11.4:** Commit and log calibration results.

*   [ ] **Task 12: Persiapan Ground Truth (D-B6)**
    *   [ ] **Step 12.1:** Expand [ground_truth.csv](file:///D:/Kuliah/Skripsi%20Repository/unsrat-rag-v4-28.05.2026/eval/dataset/ground_truth.csv) from the minimal 3 test questions to 30–50 natural language question-answer pairs covering academic regulations, vision-misi, and calendar.
...
*   [ ] **Task 13: Evaluasi Resmi & Validasi Akhir**
    *   [ ] **Step 13.1:** Clear out old test evaluation outputs (delete old result CSVs).
    ...
    *   [ ] **Step 13.6:** Generate comparative grouped bar charts: `python evaluation.py --visualize`
